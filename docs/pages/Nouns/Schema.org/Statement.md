...
A statement about something, for example a fun or interesting fact. If known, the main entity this statement is about can be indicated using mainEntity. For more formal claims (e.g. in Fact Checking), consider using <Link href="/Claim">Claim</Link> instead. Use the <a class="localLink" href="/text">text</a> property to capture the text of the statement.
