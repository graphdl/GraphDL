...
export type PropertyReference<G extends Graph<G>> = keyof G['nouns'] extends string ? (keyof G['nouns'] | `${keyof G['nouns']}[]`) : never
...
export type Property<G extends Graph<G>, N extends keyof G['nouns'], P extends keyof G['properties'][N]> = 
  PrimitiveTypes | ComplexTypes | FunctionalTypes | ThingProperties | PropertyReference<G>


export type PrimitiveTypes = 'String' | 'Number' | 'Boolean' | 'Date' | 'Time' | 'DateTime' | 'Timestamp' | 'ObjectId' | 'Array' | 'Object' | 'JSON' | 'Any'
export type ComplexTypes = 'Reference' | 'Lookup' | 'Search' | 'Formula' | 'Select' | 'SelectMany'
