...
export type Things<N extends Nouns<N>> = Possibly<SchemaThings | DefaultNouns | NounNames<N>>
...
export type Nouns<N extends Record<string, any>> = {
  [K in keyof N]: Things<N> | Is | Noun<N[K]> | Array<Things<N>>  // | PropositionalPhrase<N>> Commenting this out for now because it's causing performance issues
}

export type Is = { is: any }
export type Possibly<N extends string> = N | `${N}?`
...
export type $Context<N extends Nouns<N>, V extends Verbs<N>> = {
  ...
  stream: Stream<N,V>
  user: User<N,V>
  巛: Actions<N,V> | Events<N,V> | Stream<N,V>
  彡: Database<N,V>
